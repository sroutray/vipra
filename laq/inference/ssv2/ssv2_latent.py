...
SSV2_CONFIG = DATASET_CFG["ssv2"]
STEPSIZE = SSV2_CONFIG["stepsize"]  # Frames to skip while sampling windows


@beartype
def discover_ssv2_episodes(data_root: Path, split: Literal["train", "val", "trainval"]) -> List[Tuple[str, str, str]]:
    """Discover all SSV2 video episodes with their labels."""

    # Determine which label files to use
    if split == "train":
        label_paths = [data_root / "labels" / "train.json"]
    elif split == "val":
        label_paths = [data_root / "labels" / "validation.json"]
    elif split == "trainval":
        label_paths = [data_root / "labels" / "train.json", data_root / "labels" / "validation.json"]
    else:
        raise ValueError(f"Unknown split: {split}. Must be 'train', 'val', or 'trainval'")

    episodes = []
    frames_dir = data_root / "20bn-something-something-v2-frames"
    ...
    for label_path in label_paths:
        if not label_path.exists():
            logger.warning(f"Label file {label_path} does not exist, skipping.")
            continue
        ...
        for entry in data:
            video_id = entry["id"]
            label = entry["label"]
            template = entry.get("template", label)  # Fallback to label if no template

            # Check if video directory exists
            video_dir = frames_dir / video_id
            if video_dir.exists() and video_dir.is_dir():
                episodes.append((str(video_dir), video_id, label))
            else:
                logger.warning(f"Video directory {video_dir} does not exist, skipping video {video_id}")

    logger.info(f"Found {len(episodes)} episodes for split '{split}'")
    return episodes


def load_ssv2_episode_data(episode_info: Tuple[str, str, str], dataset_name: str) -> List[Dict[str, Any]]:
    """Load data for a single SSV2 episode."""
    video_dir_path, video_id, label = episode_info
    video_dir = Path(video_dir_path)

    # Get all image files and sort them numerically
    image_files = []
    for ext in ["*.jpg", "*.jpeg", "*.png"]:
        image_files.extend(video_dir.glob(ext))

    if not image_files:
        raise ValueError(f"No image files found in {video_dir}")

    # Sort by numeric part of filename (e.g., 000001.jpg, 000002.jpg)
    ...
    for i, img_file in enumerate(image_files):
        ...
        entry = {
            "id": f"{dataset_name}/{video_id}/{step_id}",
            "image": str(img_file),
            "raw_action": [0.0] * 7,  # SSV2 doesn't have actions, use dummy 7-dim action
            "instruction": f"<s> You are a helpful assistant. USER: What action should the robot take to `{label}` ASSISTANT:",
        }
...
def _load_single_ssv2_episode_wrapper(args: Tuple[Tuple[str, str, str], str]) -> List[Dict[str, Any]]:
    """Wrapper function for parallel episode loading - needs to be top-level for pickling."""
    episode_info, dataset_name = args
    try:
        return load_ssv2_episode_data(episode_info, dataset_name)
    except Exception as e:
        # Can't use logger here due to multiprocessing, will be handled in main function
        return []


def load_ssv2_episodes_parallel(
    episode_infos: List[Tuple[str, str, str]], dataset_name: str, max_workers: int = None
) -> List[List[Dict[str, Any]]]:
    """Load multiple SSV2 episodes in parallel using ProcessPoolExecutor."""
    logger.info(f"Loading {len(episode_infos)} SSV2 episodes with {max_workers} parallel workers...")

    all_episodes_data = []
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all episode loading tasks
        future_to_info = {
            executor.submit(_load_single_ssv2_episode_wrapper, (ep_info, dataset_name)): ep_info
        ...
        for future in concurrent.futures.as_completed(future_to_info):
            ep_info = future_to_info[future]
            try:
                ep_data = future.result()
                if ep_data:  # Only add non-empty episodes
                    all_episodes_data.append(ep_data)
                else:
                    logger.warning(f"Empty episode data for: {ep_info[1]}")  # video_id

                completed += 1
                if completed % 100 == 0:  # Log progress every 100 episodes
                    logger.info(f"Loaded {completed}/{len(episode_infos)} episodes...")

            except Exception as e:
                logger.error(f"Exception loading episode {ep_info[1]}: {e}")  # video_id

    logger.info(f"Successfully loaded {len(all_episodes_data)} out of {len(episode_infos)} episodes")
...
class SingleEpisodeDataset(Dataset):
    ...
    def _window_indices(self, t: int) -> List[int]:
        # [t-H_p*stepsize, ..., t, ..., t+H_f*stepsize] with repeat-padding
        # Apply stepsize to create temporal gaps between frames
        start = t - self.H_p * self.stepsize
        return [self._clamp(start + k * self.stepsize) for k in range(self.W)]
...
def main():
    parser = argparse.ArgumentParser(description="Process SSV2 dataset for latent action quantization")
    parser.add_argument(
        "--split", type=str, required=True, choices=["train", "val", "trainval"], help="SSV2 split to process"
    )
    ...
    # Set up output directory based on split
    SAVE_ROOT = Path("/fsx2/shared/sroutray/vipra_data") / f"ssv2_{args.split}"
    ...
    logger.info(f"Processing SSV2 split: {args.split}")
    ...
    # Get SSV2 data root from config
    data_root = Path(SSV2_CONFIG["root_dir"])

    # Discover all episodes for the specified split
    episode_infos = discover_ssv2_episodes(data_root, args.split)
    logger.info(f"Found {len(episode_infos)} episodes")

    # Shard episodes across processes
    my_episodes = [ep for i, ep in enumerate(episode_infos) if i % world == rank]
    ...
    my_episodes_data = load_ssv2_episodes_parallel(
        my_episodes, f"ssv2_{args.split}", max_workers=episode_workers_per_rank
    )
    ...
    for ep_data in tqdm(my_episodes_data, desc=f"Rank {rank} Processing Episodes"):
        ...
        ep_ds = SingleEpisodeDataset(ep_data, H_P, H_F, stepsize=STEPSIZE)
        ep_dl = DataLoader(
            ep_ds,
            batch_size=160,
            shuffle=False,
