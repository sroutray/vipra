...
from policy.dynamics_action_cont_llama import VideoLLaMAConfig
from pydantic import BaseModel
from samplers.sampler_dynamics_action_cont import ActionContSampler
...
class ViPRAConfig:
    ...
    dtype: str = "bf16"
    ckpt_path: str = (
        "trainstate_params::/mnt/home/sroutray/vipra_project/vipra/outputs/vipra_finetune_libero_long_ca14_run2/streaming_train_state"
    )
    ...
    update_llama_config: str = (
        "dict(action_dims=7,latent_action_vocab_size=9,sample_mode='text',theta=50000000,max_sequence_length=32768,scan_attention=False,scan_query_chunk_size=1024,scan_key_chunk_size=1024,scan_mlp=False,scan_mlp_chunk_size=8192,scan_layers=True)"
    )
...
class ViPRAInference:
    def __init__(self, task_description: str, **kwargs) -> None:
        ...
        self.model = ActionContSampler(FLAGS=flags)
    ...
    def step(self, image_list: List[np.ndarray], task_description: Optional[str] = None) -> np.ndarray:
        ...
        prompts = [{"image": image_prompt, "question": task_description}]
...
def startup_event():
    ...
    try:
        ...
        _ = inference_model.step([d0, d1], "compilation_trigger")
...
def step_endpoint(request: StepRequest):
    ...
    try:
        ...
        actions = inference_model.step(image_list=image_list, task_description=request.task_description)
