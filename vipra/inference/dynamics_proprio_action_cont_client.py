...
class ViPRAClient:
    ...
    def __init__(
        ...
        proprio_dims: int = 15,  # match server
        proprio_history_len: int = 7,  # match server
    ):
    ...
    def get_action(
        ...
        if mode == "json":
            return self._get_action_json(image_list, proprio_list, task_description)
        else:
            return self._get_action_bytes(image_list, proprio_list, task_description)

    # ----------------- Mode: JSON -----------------

    def _get_action_json(
        self, image_list: List[np.ndarray], proprio_list: List[np.ndarray], task_description: Optional[str]
    ) -> np.ndarray:
        images_payload = [self._prep_image_for_json(img) for img in image_list]
        proprio_payload = [self._prep_proprio_for_json(p) for p in proprio_list]
        payload = {
            "image": images_payload,
            "proprio": proprio_payload,
            "task_description": task_description or self.current_task,
        }
    ...
    def _prep_proprio_for_json(self, proprio: np.ndarray) -> list:
        if not isinstance(proprio, np.ndarray) or proprio.dtype != np.float32:
            raise ValueError("Each proprio must be a float32 numpy array")
        if proprio.ndim != 1 or proprio.shape[0] != self.proprio_dims:
            raise ValueError(f"Each proprio must be of shape ({self.proprio_dims},)")
        return proprio.tolist()

    # ----------------- Mode: BYTES (multipart) -----------------

    def _get_action_bytes(
        self, image_list: List[np.ndarray], proprio_list: List[np.ndarray], task_description: Optional[str]
    ) -> np.ndarray:
        ...
        # Prep proprio (raw float32 bytes in C-order)
        proprio_bytes = self._prep_proprio(proprio_list)

        files = [
            ("image0", ("img0.jpg", img0_bytes, "image/jpeg")),
            ("image1", ("img1.jpg", img1_bytes, "image/jpeg")),
            ("proprio", ("proprio.bin", io.BytesIO(proprio_bytes), "application/octet-stream")),
        ]
    ...
    def _prep_proprio(self, proprio_list: List[np.ndarray]) -> bytes:
        proprio_arr = np.ascontiguousarray(np.stack(proprio_list, axis=0), dtype=np.float32)
        if proprio_arr.shape != (self.proprio_history_len, self.proprio_dims):
            raise ValueError(f"Proprio array must be of shape ({self.proprio_history_len}, {self.proprio_dims})")
        return proprio_arr.tobytes(order="C")
...
if __name__ == "__main__":
    ...
    proprio_norm_stats_path = "/fsx2/shared/sroutray/LIBEROv2/libero_10_modified/proprio_normalization_stats_v2.json"
    traj_id = "ep00118/"
    traj_data = []
    ...
    with open(proprio_norm_stats_path, "r") as f:
    ...
    for step, data in tqdm(enumerate(traj_data)):
        ...
        proprio_hist = (
            np.array(data["proprio"]).reshape(proprio_history_len, proprio_dims).astype(np.float32)
        )  # already normed
        proprio_list = [proprio_hist[i] for i in range(proprio_history_len)]
        # import ipdb; ipdb.set_trace()
        img_arr = np.array(img)
        state_buffer.append(img_arr)
        if curr_action_sequence is None or curr_action_index >= pred_horizon:
            # Get action from agent
            pred_action_sequence = agent.get_action(
                [state_buffer[0], state_buffer[-1]], proprio_list, task_description, mode="bytes"
            )
