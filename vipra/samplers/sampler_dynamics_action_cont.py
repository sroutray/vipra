...
from policy.dynamics_action_cont_llama import FlaxVideoLLaMAForCausalLM, VideoLLaMAConfig
...
class CenterCropThenResize(ImageOnlyTransform):
    ...
    def apply(self, img, **params):
        ...
        return albumentations.Resize(self.height, self.width).apply(cropped_img, interpolation=self.interpolation)
...
class ActionContSampler:
    ...
    @cached_property
    def _forward_generate_latent_state(self):
        def fn(params, rng, batch, n_tokens):
            batch = with_sharding_constraint(batch, PS(("dp", "fsdp"), "sp"))
            time = np.zeros((batch["input_ids"].shape[0],), dtype=np.float32)  # time is a scalar
            time = with_sharding_constraint(time, PS(("dp", "fsdp")))
            rng_generator = JaxRNG(rng)

            action_output = self.model.generate(
                batch["input_ids"],
                n_tokens_per_sample=257,
                vision_masks=batch["vision_masks"],
                attention_mask=batch["attention_mask"],
                latent_action_masks=batch["latent_action_masks"],
                latent_state_masks=batch["latent_state_masks"],
                ...
                generation_config=GenerationConfig(
                    ...
                    pad_token_id=self.eov_token,
                    eos_token_id=self.eov_token,
                    forced_eos_token_id=self.eov_token,
                ),
                sample_mode="latent_state",
            ).sequences
            action_output = action_output[:, batch["input_ids"].shape[1] :]
            return action_output, rng_generator()

        return pjit(fn, in_shardings=(self.model_ps, PS(), PS()), out_shardings=(PS(), PS()), static_argnums=(3,))

    def generate_latent_state_action_pred(self, prompts, images, max_input_length):
        ...
        batch = dict(
            ...
            attention_mask=np.concatenate(
                [inputs.attention_mask, np.ones(images.shape, dtype=inputs.attention_mask.dtype)], axis=1
            ),
            ...
            actions=np.concatenate(
                [
                    np.zeros(
                        (inputs.input_ids.shape[0], inputs.input_ids.shape[1], self.action_dims), dtype=np.float32
                    ),
                    np.zeros((images.shape[0], images.shape[1], self.action_dims), dtype=np.float32),
                ],
                axis=1,
            ),
        )
        with self.mesh:
            latent_state, sharded_rng = self._forward_generate_latent_state(self.params, sharded_rng, batch, 257)
            latent_state = jax.device_get(latent_state)

        # append the latent state token to the input for action generation
        prefix_for_gen = ["</vision> <action>"] * len(prompts)
        inputs_for_gen = self.prefix_tokenizer(prefix_for_gen, return_tensors="np")
        batch = dict(
            input_ids=np.concatenate([batch["input_ids"], latent_state, inputs_for_gen.input_ids], axis=1),
            attention_mask=np.concatenate(
                [
                    batch["attention_mask"],
                    np.ones(latent_state.shape, dtype=batch["attention_mask"].dtype),
            ...
            vision_masks=np.concatenate(
                [
                    batch["vision_masks"],
                    np.zeros(latent_state.shape, dtype=bool),
            ...
            latent_state_masks=np.concatenate(
                [
                    batch["latent_state_masks"],
                    np.ones(latent_state.shape, dtype=bool),
                    np.zeros(inputs_for_gen.input_ids.shape, dtype=bool),
                ],
                axis=1,
            ),
        ...
        # print(f"Action output shape: {action_output_mutable.shape}")
        # print(f"Action output: {action_output_mutable}")
        latent_state_image = self.encoding_to_image(latent_state)
        return action_output_mutable, latent_state_image

    def generate_action_pred(self, prompts, images, max_input_length):
    ...
    def __call__(self, prompts):
        ...
        action_output = self.generate_action_pred(
            prompts=[text_prompt], images=batch["input_ids"], max_input_length=128
        )
