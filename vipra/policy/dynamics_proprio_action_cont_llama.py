...
class Encoder(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.linear_2 = nn.Dense(
            self.width // 2 if not self.cond else self.width,
            use_bias=True,
            dtype=self.dtype,
            param_dtype=self.param_dtype,
            precision=self.precision,
    ...
    def __call__(self, action: jnp.ndarray, cond_emb: Optional[jnp.ndarray] = None) -> jnp.ndarray:
        # [Batch_Size, Seq_Len, Width]
        emb = self.linear_1(action)
        if self.cond and cond_emb is not None:
            # repeat cond embedding for seq_len
            # [Batch_Size, Seq_Len, Width]
            cond_emb_full = jnp.expand_dims(cond_emb, axis=1)
            cond_emb_full = jnp.tile(cond_emb_full, (1, action.shape[1], 1))
            emb = jnp.concatenate([cond_emb_full, emb], axis=-1)
        ...
        if scan_layers:
            if scan_axis == 0:
                return (
                    ...
                    # proprio encoder
                    ("transformer/proprio_encoder/linear_1/kernel", PS(None, "tp")),
                    ("transformer/proprio_encoder/linear_1/bias", PS("tp")),
                    ("transformer/proprio_encoder/linear_2/kernel", PS("tp", None)),
                    ("transformer/proprio_encoder/linear_2/bias", PS(None)),
                    ("transformer/proprio_encoder/linear_3/kernel", PS(None, ("fsdp", "sp"))),
                    ("transformer/proprio_encoder/linear_3/bias", PS(("fsdp", "sp"))),
                    # action encoder
...
class FlaxVideoLLaMAPreTrainedModel(FlaxPreTrainedModel):
    ...
    def init_cache(self, batch_size, max_length):
        ...
        proprio = jnp.zeros((batch_size, max_length, self.config.proprio_dims), dtype="f4")
        ...
        proprio_masks = jnp.ones((batch_size, max_length), dtype=bool)
    ...
    def init_weights(self, rng, input_shape, params=None):
        ...
        proprio = jnp.zeros((input_shape[0], input_shape[1], self.config.proprio_dims), dtype="f4")
        ...
        proprio_masks = jnp.ones(input_ids.shape, dtype=bool)
    ...
    def __call__(
        ...
        outputs = self.module.apply(
            ...
            jnp.array(proprio_masks, dtype="f4"),
...
class FlaxVideoLLaMAModule(nn.Module):
    ...
    def setup(self):
        ...
        self.proprio_encoder = Encoder(
            self.config.proprio_dims,
            self.config.hidden_size,
            cond=False,
            dtype=self.dtype,
            param_dtype=self.param_dtype,
    ...
    def __call__(
        ...
        if input_ids.shape[1] == 1 or (input_ids.shape[1] != 1 and self.config.sample_mode == "action"):
            ...
            proprio_masks = proprio_masks[..., None].astype("f4")  # 1 is proprio, 0 is others
            ...
            input_embeds = input_embeds * (1 - proprio_masks) + proprio_embeds * proprio_masks
...
class FlaxDynamicsProprioActionContLLaMAForCausalLMModule(nn.Module):
    config: VideoLLaMAConfig
    dtype: jnp.dtype = jnp.float32
    param_dtype: jnp.dtype = jnp.float32
    precision: Optional[Union[jax.lax.Precision, str]] = None
...
class FlaxVideoLLaMAForCausalLM(FlaxVideoLLaMAPreTrainedModel):
    module_class = FlaxDynamicsProprioActionContLLaMAForCausalLMModule
    ...
    def update_inputs_for_generation(self, model_outputs, model_kwargs):
        return {
            ...
            "proprio_masks": model_kwargs["proprio_masks"],
            "action_masks": model_kwargs["action_masks"],
            "proprio": model_kwargs["proprio"],
    ...
    def generate_action_naive(
        ...
        **kwargs,
        ...
        Args:
            ...
            proprio: (B, T_proprio, D_proprio)
            actions: (B, T_action, D_action) or None
        ...
        proprio_dim = self.config.proprio_dims
        ...
        proprio_masks = expand_and_concat(kwargs["proprio_masks"], fill=0)
        ...
        full_proprio = lax.dynamic_update_slice(jnp.zeros((batch_size, max_length, proprio_dim)), proprio, (0, 0, 0))
    ...
    def generate_action_kv_cache(
        ...
        **kwargs,
        ...
        full_proprio_masks = expand_and_concat(kwargs["proprio_masks"], fill=0)
        ...
        init_state = self.prepare_inputs_for_generation(
            ...
            proprio_masks=full_proprio_masks,
        ...
        def euler_step(state, _):
            ...
            out = self(
                ...
                proprio_masks=gen_proprio_masks,
