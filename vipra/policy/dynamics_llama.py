...
class VideoLLaMAConfig(LLaMAConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.latent_action_vocab_size = latent_action_vocab_size  # last token is padding token
    ...
    def __call__(
        ...
        if input_ids.shape[1] == 1:
            if self.config.sample_mode == "text":
                ...
                raise NotImplementedError("All sampling not supported for single token")
            else:
...
class FlaxDynamicsLLaMAForCausalLMModule(nn.Module):
    config: VideoLLaMAConfig
    dtype: jnp.dtype = jnp.float32
    param_dtype: jnp.dtype = jnp.float32
    precision: Optional[Union[jax.lax.Precision, str]] = None
    ...
    def __call__(
        ...
        if self.config.sample_mode == "all":
            if not return_dict:
                return (vision_logits, lm_logits, latent_action_logits, latent_state_logits) + outputs[1:]

            return FlaxCausalLMOutput(
                logits=(vision_logits, lm_logits, latent_action_logits, latent_state_logits),
...
class FlaxVideoLLaMAForCausalLM(FlaxVideoLLaMAPreTrainedModel):
    module_class = FlaxDynamicsLLaMAForCausalLMModule
